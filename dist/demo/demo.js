...
import { max, addHours, addSeconds, addMinutes, differenceInSeconds, setHours, setMinutes, setSeconds, setMilliseconds, setDay } from 'date-fns';
...
export class Demo extends React.PureComponent {
  ...
  onEventDrop(droppedEventId, layerName, timeEventDroppedOn) {
    ...
    if (newEvent.start.getDay() !== newEvent.end.getDay()) return;
  ...
  onEventResize(droppedEventId, timeEventResizedTo, resizeType) {
    ...
    const newEvent = Object.assign({}, droppedEvent);
    if (newEvent.start.getDay() !== newEvent.end.getDay()) return;
    if (newEvent.start.getDay() !== timeEventResizedTo.getDay()) return;
  ...
  onCalendarClick(timeClickedOn) {
    ...
    if (start.getDay() !== end.getDay()) return;
    const newEvent = {
      id: "created-event" + this.state.numCreated,
      start: start,
      end: end
    };
    this.setState({
      numCreated: this.state.numCreated + 1,
  ...
  render() {
    ...
    const layers = [{
      ...
      name: PREFERENCE,
      events: preferenceComponents,
      eventClassName: PREFERENCE
    }];

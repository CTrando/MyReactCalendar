...
export class EventLayerOuterInator extends React.PureComponent {
  ...
  getEventColumnStyle(event, eventColumnMap, columnIndex) {
    ...
    for (; endIndex < eventColumnMap.length; endIndex++) {
      const eventsPerColumn = eventColumnMap[endIndex]; // check if events in the next column are contained by our current column
  ...
  getEventStyle(event, eventColumnMap, columnIndex) {
    ...
    const startTime5MinuteIntervals = Math.floor((eventStart.getHours() - this.props.startHour) * INTERVALS_PER_HOUR) + Math.floor(eventStart.getMinutes() / INTERVALS) + 1;
    const endTime5MinuteIntervals = Math.floor((eventEnd.getHours() - this.props.startHour) * INTERVALS_PER_HOUR) + Math.floor(eventEnd.getMinutes() / INTERVALS) + 1;
    return {
  ...
  layoutEventsIntoColumns(events) {
    ...
    let columns = []; // for each event we will determine what column we should put it in

    for (let event of events) {
      let curColumn = null; // loop over the existing columns
  ...
  styleEventsInColumns(eventColumnMap) {
    ...
    for (let column = 0; column < eventColumnMap.length; column++) {
      ...
      for (let evt of eventsPerColumn) {
        ...
        const classNames = className(this.props.eventClassName, "event-wrapper"); // user will determine what kind of component to render based on the information given here
  ...
  layoutEventsIntoDays() {
    ...
    for (let eventsPerDay of eventsByDay) {
      ...
      if (eventsPerDay) {
        ...
        let styledEvents = this.styleEventsInColumns(eventColumnMap); // put the styled events into one day column

        const styledDays = React.createElement("div", {
          key: idx,
          style: this.getDayStyle(eventColumnMap.length)
        }, styledEvents);
        ret.push(styledDays);
      } else {
  ...
  render() {
    return React.createElement(React.Fragment, null, this.layoutEventsIntoDays());
  }
